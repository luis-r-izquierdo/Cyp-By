...
;; Cyp+By_applet
;; Cyp+By_applet is a model designed to analyse
...
to go
  ...
  ;; new generation
  kill-and-breed
  if (count turtles != num-players) [adjust-num-players]
end

to make-couples
  ask turtles [set new-partnership? false]
  ask turtles with [mate = nobody] [set new-partnership? true]
  ask turtles [
      if (mate = nobody) [
        set mate one-of turtles with [mate = nobody and self != myself]
...
to adjust-num-players
  let adjustment (num-players - (count turtles))
  if adjustment != 0 [
  ifelse adjustment > 0
    [
      crt adjustment [
...
to gather-data
  set numCC (count turtles with [active-action = 0 and [active-action] of mate = 0]) / 2
  set numDD (count turtles with [active-action = 1 and [active-action] of mate = 1]) / 2
  set numCD (count turtles with [active-action = 0 and [active-action] of mate = 1])
...
to update-graphs
  ...
  let current-num-players (count turtles)
